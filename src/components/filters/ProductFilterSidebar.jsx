...
const ProductFilterSidebar = ({
  ...
  return (
    <>
      ...
      {/* Filter Sidebar */}
      <aside
        className={`w-72 p-4 border-r bg-white self-start sticky top-24 transition-all duration-300 ${
          isMobileOpen
            ? "fixed left-0 top-0 z-50 w-72 h-screen overflow-y-auto rounded-r-lg shadow-lg"
            : "hidden md:block"
        }`}
      >
        ...
        <CollapsibleSection title="Price Range">
          <div className="flex items-center space-x-2">
            <input
              ...
              onChange={(e) =>
                onFilterChange("price", [Number(e.target.value), selectedFilters.price[1]])
              }
            ...
            <input
              ...
              onChange={(e) =>
                onFilterChange("price", [selectedFilters.price[0], Number(e.target.value)])
              }
              className="w-16 border rounded px-1"
            />
          </div>
