import prisma from "../../database/prismaClient.js";

const slugify = (value) => {
  if (!value || typeof value !== "string") return "";
  return value
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "-")
    .replace(/[^a-z0-9-]/g, "")
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "");
};

const normalizeString = (value) => (typeof value === "string" ? value.trim() : "");

const getUniqueSlug = async (baseSlug) => {
  let slug = baseSlug;
  let attempt = 0;

  while (true) {
    const existing = await prisma.category.findUnique({ where: { slug } });
    if (!existing) {
      return slug;
    }
    attempt += 1;
    slug = `${baseSlug}-${attempt}`;
  }
};

const buildCategoryResponse = (category) => {
  if (!category) return null;
  return {
    id: category.id,
    name: category.name,
    slug: category.slug,
    description: category.description ?? "",
    status: category.status,
    banner: category.banner ?? "",
    createdAt: category.createdAt,
    updatedAt: category.updatedAt,
  };
};

const buildCategoryQuery = (query = {}) => {
  const normalizedSearch = normalizeString(query.q);
  const where = { deletedAt: null };

  if (normalizeString(query.status)) {
    where.status = normalizeString(query.status);
  }

  if (normalizedSearch) {
    where.OR = [
      { name: { contains: normalizedSearch, mode: "insensitive" } },
      { slug: { contains: normalizedSearch, mode: "insensitive" } },
      { description: { contains: normalizedSearch, mode: "insensitive" } },
    ];
  }

  return where;
};

const mapCategoryOrderBy = (sortBy, order = "asc") => {
  const direction = order === "desc" ? "desc" : "asc";
  switch (sortBy) {
    case "createdAt":
      return { createdAt: direction };
    case "name":
    default:
      return { name: direction };
  }
};

const resolveCategoryWhere = (categoryId) => {
  const parsedId = Number(categoryId);
  if (Number.isInteger(parsedId) && String(parsedId) === String(categoryId)) {
    return { id: parsedId };
  }
  return { slug: String(categoryId).trim() };
};

export const getCategories = async (query = {}) => {
  const page = Number(query.page ?? 1);
  const limit = Number(query.limit ?? 10);
  const skip = Math.max(0, (page - 1) * Math.max(1, limit || 1));
  const where = buildCategoryQuery(query);
  const orderBy = mapCategoryOrderBy(query.sortBy, query.order);

  const total = await prisma.category.count({ where });
  const items = await prisma.category.findMany({
    where,
    orderBy,
    skip: limit === 0 ? undefined : skip,
    take: limit === 0 ? undefined : Math.max(1, limit),
  });

  return {
    items: items.map(buildCategoryResponse),
    total,
    page: Math.max(1, page),
    pageSize: limit === 0 ? total : Math.max(1, limit),
  };
};

export const getCategory = async (categoryId) => {
  const where = resolveCategoryWhere(categoryId);
  const category = await prisma.category.findFirst({ where: { ...where, deletedAt: null } });
  return buildCategoryResponse(category);
};

export const createCategory = async (payload = {}) => {
  const name = normalizeString(payload.name);
  const slugBase = slugify(payload.slug || name) || `category-${Date.now()}`;
  const slug = await getUniqueSlug(slugBase);

  const category = await prisma.category.create({
    data: {
      name,
      slug,
      description: normalizeString(payload.description) || null,
      status: normalizeString(payload.status) || "active",
      banner: normalizeString(payload.banner) || null,
    },
  });

  return buildCategoryResponse(category);
};

export const updateCategory = async (categoryId, payload = {}) => {
  const where = resolveCategoryWhere(categoryId);
  const existing = await prisma.category.findFirst({ where: { ...where, deletedAt: null } });
  if (!existing) return null;

  const data = {};
  if (payload.name !== undefined) {
    data.name = normalizeString(payload.name) || existing.name;
  }

  if (payload.slug !== undefined) {
    const slugBase = slugify(payload.slug || payload.name || existing.name) || `category-${Date.now()}`;
    data.slug = slugBase === existing.slug ? existing.slug : await getUniqueSlug(slugBase);
  }

  if (payload.description !== undefined) {
    data.description = normalizeString(payload.description) || null;
  }

  if (payload.status !== undefined) {
    data.status = normalizeString(payload.status) || existing.status;
  }

  if (payload.banner !== undefined) {
    data.banner = normalizeString(payload.banner) || null;
  }

  const updated = await prisma.category.update({
    where: { id: existing.id },
    data,
  });

  return buildCategoryResponse(updated);
};

export const deleteCategory = async (categoryId) => {
  const where = resolveCategoryWhere(categoryId);
  const existing = await prisma.category.findFirst({ where: { ...where, deletedAt: null } });
  if (!existing) return null;

  const deleted = await prisma.category.update({
    where: { id: existing.id },
    data: { deletedAt: new Date() },
  });

  return buildCategoryResponse(deleted);
};
