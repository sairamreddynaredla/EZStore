...
const ProductCard = ({ product, onAddToCart, onWishlistToggle, compact = false, showWishlistButton = true }) => {
  ...
  // Use real-time price if available, otherwise use product price
      ...
      {compact ? (
            ...
            <div className="mb-3">
              {originalPrice > currentPrice ? (
                <div className="flex items-baseline gap-2">
                  ...
                  <span className={`text-base font-bold text-gray-900 ${realtimePrice.isUpdating ? "animate-pulse" : ""}`}>₹{currentPrice.toFixed(2)}</span>
                </div>
              ) : (
                <div className={`text-base font-bold text-gray-900 ${realtimePrice.isUpdating ? "animate-pulse" : ""}`}>₹{currentPrice.toFixed(2)}</div>
              )}
            </div>
            <div>
        ...
        <>
          ...
          <div className="flex flex-col flex-1 px-3 sm:px-3 pb-2 sm:pb-2 pt-1 sm:pt-1">
            ...
            <div className="flex items-center justify-between mb-1 sm:mb-2"><div className="flex items-baseline gap-1 sm:gap-2">{originalPrice > currentPrice && (<span className="text-[11px] sm:text-[12px] text-gray-400 line-through hidden sm:inline">${originalPrice.toFixed(2)}</span>)}<span className={`text-lg sm:text-[18px] font-bold text-gray-900 ${realtimePrice.isUpdating ? "animate-pulse" : ""}`}>${currentPrice.toFixed(2)}</span></div>{discount > 0 && (<span className="hidden sm:inline-block bg-red-500 text-white text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded">{discount}% Off</span>)}</div>
