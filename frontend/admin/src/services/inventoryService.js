...
export const updateInventoryStock = async (productId, payload) => {
  const normalizedPayload = typeof payload === "number" ? { stock: payload } : payload ?? {};
  const response = await adminApi.put(`/products/${productId}`, normalizedPayload);
  return response.data;
};

export const getInventoryHistory = async (productId, params = {}) => {
  const response = await adminApi.get(`/products/${productId}/stock-history`, { params });
  return normalizeListResponse(response);
};
