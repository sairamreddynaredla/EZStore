...
// Remove any import lines robustly by filtering lines
const lines = original.split(/\r?\n/);
const filtered = lines.filter(l => !/^\s*import\b/.test(l));
let tmp = filtered.join('\n');
tmp = tmp.replace(/export\s+const\s+products\s*=\s*/m, 'module.exports = ');

// Remove any remaining export default lines (e.g. `export default products;`)
tmp = tmp.replace(/^\s*export\s+default.*$/gm, '');
