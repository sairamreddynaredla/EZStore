...
const sections = ["overview", "nutrition", "training", "puppy-care", "lifestyle", "faq"];
...
const BreedDetailsLayout = () => {
  ...
  return (
    <div className="w-full bg-[#f7f4ef] overflow-x-hidden">
          ...
          >
            <div
              className={`w-4 h-4 rounded-full mb-2 transition-all duration-300 ${
                activeSection === section ? "bg-orange-500 scale-110" : "bg-gray-300"
              }`}
            />
      ...
      <section id="overview" className="w-[88%] mx-auto mt-20">
      ...
      {breed.sections.map((section) => (
        <section key={section.id} id={section.id} className="w-[88%] mx-auto mt-28">
          <h2 className="text-4xl lg:text-5xl font-bold text-[#222] mb-10">{section.title}</h2>
          ...
          <div className="mt-10">
            <p className="text-[17px] leading-9 text-gray-700 mb-8">{section.content.text}</p>
            ...
            {section.content.table && (
              <div className="overflow-x-auto mt-10 rounded-3xl">
                <table className="w-full bg-white overflow-hidden">
                  ...
                  <tbody>
                    {section.content.table.rows.map((row, index) => (
                      <tr key={index} className="border-b">
                        {row.map((cell, cellIndex) => (
                          <td key={cellIndex} className="p-5 text-gray-700">
                            {cell}
                          </td>
                        ))}
                      </tr>
      ...
      <section className="w-[88%] mx-auto mt-28">
        <h2 className="text-5xl font-bold mb-10">Fun Trivia: Did You Know?</h2>

        <div className="space-y-5">
          {breed.trivia.map((fact, index) => (
            <div key={index} className="bg-white rounded-3xl p-8 flex gap-6">
              <span className="text-3xl font-bold text-orange-500">0{index + 1}</span>

              <p className="text-gray-700 leading-8 text-lg">{fact}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* FAQ */}
      <section id="faq" className="w-[88%] mx-auto mt-28">
        <h2 className="text-5xl font-bold mb-10">FAQs</h2>

        <div className="space-y-5">
          {breed.faqs.map((faq, index) => (
            <details key={index} className="bg-white rounded-3xl p-7">
              <summary className="cursor-pointer text-lg font-semibold">{faq.q}</summary>

              <p className="mt-5 text-gray-700 leading-8">{faq.a}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      <section className="w-[88%] mx-auto mt-28 pb-24">
        <h2 className="text-5xl font-bold mb-10">Related Breeds</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {breed.breedsList.map((item, index) => (
            <div
              key={index}
            ...
            >
              ...
              <h4 className="p-5 text-lg font-semibold">{item.name}</h4>
            </div>
          ))}
        </div>
      </section>
